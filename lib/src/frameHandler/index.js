...
module.exports = class FrameHandler {
  ...
  constructor(frame, ctx) {
    ...
    const { DESIoTConsole } = require('../../src');
    this.DESIoTConsole = DESIoTConsole;
  }
  async parseFrame(encrypt_en = true) {
    ...
    if (
      ...
      this.DESIoTConsole.log('\t- HEADERS OK');
      this.DESIoTConsole.log('\t- TRAILERS OK');
    }
    ...
    if (this.crc !== crcCalculate)
      ...
      this.DESIoTConsole.log('\t- CRC OK');
    }

    if (DESIOT_FRAME.ENCRYPT_ENABLE && encrypt_en) this.decryptFrame();
  ...
  async handleSuccessFrame() {
    this.DESIoTConsole.timeEnd(this.labelTime);
      ...
      case DESIOT_FRAME.CMD_DEVICE_ASSIGN_VIRTUAL_STORAGE:
        ...
        switch (vs_type) {
          ...
          case DESIOT_FRAME.VS_TYPE_STRING:
            const restDataBuf = this.data.slice(
              DESIOT_FRAME.MAIN_DATA_START_POS + 1
            );
            this.vs_data = new TextDecoder().decode(restDataBuf);
            break;
          case DESIOT_FRAME.VS_TYPE_FLOAT_ARR:
            {
              const restDataBuf = this.data.slice(
                DESIOT_FRAME.MAIN_DATA_START_POS + 1
              );
              const int32ArrBuf = new Float32Array(restDataBuf.buffer);
              // console.log(int32ArrBuf);
              this.vs_data = int32ArrBuf;
            }
            break;
        }
  ...
  async decryptFrame() {
    ...
    const [computedTag, decryptData, uTime] = await chacha20poly1305_run(
      dataFrame,
      CC20_P1305_Mode.DECRYPT
    );
    this.DESIoTConsole.log('\t- DECRYPT time: %dus', uTime);
    ...
    } else this.DESIoTConsole.log('Tag unmatched!!', computedTag, checkTag);
  }
};
