...
module.exports = class FrameHandler {
  ...
  constructor(frame, ctx) {
    ...
    this.data = this.dataPacket.subarray(3);
    // console.log(frame);
  ...
  async parseFrame() {
    ...
    const crcCalculate = await this.crcCalculate(
      this.dataPacket,
    ...
    console.log('Frame ok ', this.frame);
   ...
   */
  async crcCalculate(bytes, BYTES_LEN) {
  ...
  async handleSuccessFrame() {
    switch (this.cmd) {
      case DESIOT_FRAME.CMD_ASSIGN_VIRTUAL_STORAGE:
        const gateway_idU8Arr = this.data.slice(
          0,
          0 + DESIOT_FRAME.GATEWAY_ID_SIZE
        );
        this.gateway_id = Buffer.from(gateway_idU8Arr).toString('hex');
        ...
        const { gateway_id, connection_type, connection_id } = this;
        console.log(
          await this.ctx.service.device.findOne({
            gateway_id,
            connection_type,
            connection_id,
          })
