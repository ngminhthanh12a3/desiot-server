  ...
  async handleSuccessFrame() {
    ...
    switch (this.cmd) {
      case DESIOT_FRAME.CMD_DEVICE_ASSIGN_VIRTUAL_STORAGE:
        // console.log(await this.ctx.service.configs.findOne(this.config_id));
        this.vs_id = this.data[DESIOT_FRAME.MAIN_DATA_START_POS];
        // const { type: vs_type } = this.ctx.service.VStorage.findOne({
        //   _id: this.vs_id,
        //   config_id: this.config_id,
        // });

        this.vs_type = new Int32Array(
          this.data.slice(
        ...
        // const { config_id, vs_id, device_id, vs_value } = this;
        // this.ctx.service.VStorage.findOneAndUpdate(
        //   { config_id, vs_id },
        //   { $set: { [`data.${device_id}`]: vs_value } }
        // );
        break;
